...
class CustomGAT(MessagePassing):
    ...
    def message(self, q_i: Tensor, k_j: Tensor, v_j: Tensor, size_i, edge_index_i, edge_index_j) -> Tensor:
        ...
        # print("num nodes:", size_i, "v:", v_j.size(), "att weights:", attn_output_weights.size())
        # print("sum of all node scores:", torch.sum(attn_output_weights, dim=0))
        # for i in range(10):  # first 10 nodes
        #     node_indices = (edge_index_i == i).nonzero(as_tuple=True)[0]
        #     # print("node_zeros_indices:", node_indices)
        #
        #     scores = torch.index_select(attn_output_weights, dim=0, index=node_indices)
        #     # print("i", i, "scores:", scores.size(), scores)
        #     print("i", i, "sum_scores:", torch.sum(scores, dim=0))
