...
class DocumentStructureNode(SpanNode):
    ...
    def __init__(self, document_extract: DocumentExtract, source=construction.CONTEXT, subtype=None):
        super().__init__(document_extract, subtype=subtype, source=source)

        if self.token_span.level == construction.TOKEN:
            raise Exception()

    def get_node_viz_text(self):
        if self.token_span.level == construction.SENTENCE:
            return super().get_node_viz_text()
        if self.token_span.level == construction.QUERY_SENTENCE:
