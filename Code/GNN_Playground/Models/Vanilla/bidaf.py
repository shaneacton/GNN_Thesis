...
from Code.GNN_Playground.Models.Layers.seq2cand import Seq2Cand
...
class BiDAF(nn.Module):
    ...
    def forward(self, context: Context, question: Question):
        ...
        attended_vec = self.att_flow_layer(context.get_context_embedding().to(device), question.get_embedding().to(device))
        ...
        # 3. Output Layer
        return self.get_output(attended_vec, modeled_vec, question)

    def get_output(self, attended_vec, modeled_vec, question: Question):
        ...
        if question.get_answer_type() == OneWordAnswer:
            return self.candidate_output(modeled_vec, question.get_candidates_embedding())
