...
class LayerConstructor:
    ...
    def get_layer(self, sizes: List[int], layer_args: Dict, global_args: Dict):
        if Code.constants.LAYER_TYPE in layer_args:
            # is a predefined layer
            layer = self.get_layer_from_type(sizes, layer_args, global_args)
        else:  # must be built from modules
            prep_modules = self.get_layer_modules(Code.constants.PREPARATION_MODULES, sizes, layer_args, global_args)
            message_modules = self.get_layer_modules(Code.constants.MESSAGE_MODULES, sizes, layer_args, global_args)
            update_modules = self.get_layer_modules(Code.constants.UPDATE_MODULES, sizes, layer_args, global_args)
    ...
    def get_layer_from_type(self, sizes, layer_args, global_args):
        ...
        layer_type: Type[GraphLayer] = layer_args[Code.constants.LAYER_TYPE]
    ...
    def get_module(self, sizes, module_args, global_args):
        ...
        module_type = args[Code.constants.MODULE_TYPE]
