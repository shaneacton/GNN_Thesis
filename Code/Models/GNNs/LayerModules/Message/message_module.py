...
class MessageModule(LayerModule):
    ...
    def get_positional_embeddings(self, edge_index_i, edge_index_j, encoding) -> torch.Tensor:
        ...
        for e in range(edge_index_i.size(0)):
            try:
                pos_i = encoding.graphs.node_positions[edge_index_i[e]]
                pos_j = encoding.graphs.node_positions[edge_index_j[e]]
            except Exception as ex:
                print("e_id_i: " + repr(edge_index_i.size()) + " e_id_j: " + repr(edge_index_j.size()))
                print("i[e]:", edge_index_i[e], "num node positions:", len(encoding.graphs.node_positions), "e:", e)
        ...
        return self.get_relative_positional_embedder(encoding.graphs.gcc, encoding.gec)(relative_positions)