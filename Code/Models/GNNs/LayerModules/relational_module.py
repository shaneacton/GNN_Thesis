...
class RelationalModule(LayerModule):
    ...
    def handle_types(self, types: torch.Tensor):
        """
        if this forward pass has enountered a new type not previously seen
    ...
    def forward(self, x: torch.Tensor, types: torch.Tensor):
        ...
        w = torch.matmul(self.coefficients, self.basis.view(self.num_bases, -1))
        w = w.view(self.num_relations, self.in_channels, self.out_channels)
        w = torch.index_select(w, 0, types)  # (E/N, i, o)
        # print("w:",w.size(), "x:", x.size(), "x unsqueezed:", x.unsqueeze(1).size())
        # bmm ~ (b,n,m) * (b,m,p) = (b,n,p)
        out = torch.bmm(x.unsqueeze(1), w).squeeze(-2)  # (E/N, o)
        return out
