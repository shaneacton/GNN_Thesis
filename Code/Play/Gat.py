...
class Gat(GATConv):
    ...
    def __init__(self, in_channels: Union[int, Tuple[int, int]], out_channels: int, num_edge_types=1, edge_emb_dim=10, **kwargs):
        ...
        self.test = nn.Linear(out_channels,  out_channels)
    ...
    def message(self, x_j: Tensor, alpha_j: Tensor, alpha_i: OptTensor,
        ...
        # super_pos = torch.cat([x_j.squeeze(), type_embs], dim=1)

        x_j = self.act(self.type_map(super_pos)).view(x_j.size(0), 1, x_j.size(-1))
        # x_j = self.act(self.test(self.act(self.test(x_j))))
    ...
    #         with torch.no_grad():
    #             indices = (self._temp_edge_types == type).nonzero().squeeze().detach()
    ...
    #         #     break
    #         # break
    #     #     tf.append(transformed_features)
    #     # x_j = torch.cat(tf)
    #     return x_j
