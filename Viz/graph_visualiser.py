...
def render_graph(graph: HDEGraph, graph_name="temp", graph_folder=None, view=True):
    ...
    for i, node in enumerate(graph.ordered_nodes):
        ...
        dot.node(name_i(i), text, fillcolor=get_node_colour(node), style="filled")

    for edge in graph.ordered_edges:
        # if edge.type() == ENTITY:
        #     continue
        if name_i(edge.from_id) in ignored_nodes or name_i(edge.to_id) in ignored_nodes:
            continue
        dot.edge(name_i(edge.from_id), name_i(edge.to_id), label=edge.type())
